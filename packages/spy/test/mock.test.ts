...
describe('Mocking Utilities', () => {
    ...
    describe('mock.mockFn()', () => {
        it('should create a mock function', () => {
            ...
            expect((mockFn as any).__isMockFn).toBe(true); // Check internal flag
    ...
    describe('mock.verify()', () => {
        ...
        it('should verify a call was made with specific arguments', () => {
            ...
            expect((error as Error).message.includes('Expected \'myMock\' to be called with')).toBe(true);
            expect((error as Error).message.includes('[3,"c"]')).toBe(true);
            expect((error as Error).message.includes('But calls were:')).toBe(true);
            expect((error as Error).message.includes('[1,"a"]')).toBe(true);
            expect((error as Error).message.includes('[2,"b"]')).toBe(true);
        });
